...
static int percpu_buffer_init(struct hook_buffer_info *info, u32 fieldcnt,
...
static u64 hook_get_clock(void *info, u32 hk_id)
{
	switch (hk_id) {
		AP_HOOK_LIST
	default:
...
static void hook_info_parser(struct file *fp, struct sort_info *info)
{
	bool error;
	void *data = info->data;
 ...
 */
static int ap_hook_install(enum hook_type hk)
{
	if (hk >= HK_MAX) {
 ...
 */
static int ap_hook_uninstall(enum hook_type hk)
{
	if (hk >= HK_MAX) {
		BB_ERR("hook type [%d] is invalid!\n", hk);
		return -EINVAL;
