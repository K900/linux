...
static int hw_bp_thread_handler(void *data)
{
...
static void hw_bp_perf_handler(struct perf_event *bp, struct perf_sample_data *data,
			struct pt_regs *regs)
{
	int *hw_step = NULL;
	hw_bp_attr *attr = HW_GET_BP_ATTR(bp);
...
/*release bp*/
static void hw_bp_manage_deinit(void)
{
	int i = 0;
...
/*bp arch init*/
static int hw_bp_manage_init(void)
{
	int cpu = -1, i = 0;
